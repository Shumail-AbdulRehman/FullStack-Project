...
const generateNewAccessToken = asyncHandler(async (req, res) => {
    ...
    try {
        decodedIncomingRefreshToken = jwt.verify(
    ...
    if (user.refreshToken !== incomingRefreshToken) {
        throw new ApiError(
            401,
            "user refreshToken doesnt matches with incoming refreshToken"
        );
    }
    ...
    res.status(200)
        ...
        .json(
            new ApiResponse(
                ...
                "new accessToken and refreshToken generated successfully"
            )
        );
});
...
const updateCoverImage = asyncHandler(async (req, res) => {
    ...
    if (oldUserCoverImage) {
        try {
            const oldUserCoverImageDeleted =
                await deleteImageByUrl(oldUserCoverImage);
...
const getWatchHistory = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(
        new ApiResponse(
            200,
            getUserWatchHistory,
            "user watch history sent successfully"
        )
    );
});
...
const addWatchHistory = asyncHandler(async (req, res) => {
    ...
    if (!createWatchHistory)
        throw new ApiError(
            500,
            "soemthing went wrong while creating watch history"
        );
