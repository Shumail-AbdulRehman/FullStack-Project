...
const toggleSubscription = asyncHandler(async (req, res) => {
  ...
  if (!isUserSubscribed) {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, true, "User successfully subscribed to the channel"));
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, false, "User unsubscribed successfully"));
});
...
const getUserChannelSubscribers = asyncHandler(async (req, res) => {
  ...
  if (!userChannelSubscribers.length) {
    return res
      .status(200)
      .json(new ApiResponse(200, [], "No user is subscribed to your channel"));
  }

  return res
    .status(200)
    .json(new ApiResponse(200, userChannelSubscribers, "Subscribed user list fetched successfully"));
});
...
const getSubscribedChannels = asyncHandler(async (req, res) => {
  ...
  if (!subscriberChannels.length) {
    return res
      .status(200)
      .json(new ApiResponse(200, [], "User has not subscribed to any channels"));
  }

  return res
    .status(200)
    .json(new ApiResponse(200, subscriberChannels, "Subscribed channels fetched successfully"));
});
