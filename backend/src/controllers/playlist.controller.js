...
const createPlaylist = asyncHandler(async (req, res) => {
    ...
    if(!createdUserPlaylist) throw new ApiError(500,"something went wrong while creating user playlist");

    res.status(200)
    .json(
        new ApiResponse(200,createdUserPlaylist,"user playlist created successfully")
    )
})
...
const getUserPlaylists = asyncHandler(async (req, res) => {
    ...
    if(userPlaylists.length===0) return res.status(200).json(new ApiResponse(200,userPlaylists,"no playlist is found"))
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
    ...
    if(!(playlistId && videoId)) throw new ApiError(400,"playlist id and video id is required")

    const userPlaylist=await Playlist.findById(playlistId)

    if(!userPlaylist) throw new ApiError(404,"playlist not found")

    if(userId.toString() !== userPlaylist.owner.toString()) throw new ApiError (401,"unauthorize access")
    ...
    if(videoExistInPlaylist.length!==0) throw new ApiError(403,"video already exist in playlist")
    ...
    res.status(201)
    .json(
        new ApiResponse(201,userPlaylist,"video added to playlist successfully")
    )
...
const deletePlaylist = asyncHandler(async (req, res) => {
    ...
    if(userPlaylist.owner.toString() !== req.user._id.toString()) throw new ApiError(401,"unauthorize access")
    
    const deleteUserPlaylist=await Playlist.findByIdAndDelete(playlistId)

    if(!deleteUserPlaylist) throw new ApiError(500,"something went wrong while deleting user Playlist ")

    res.status(200).json(
        new ApiResponse(200,true,"playlist deleted")
    )
})
